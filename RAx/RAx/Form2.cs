...
namespace RAx
{
    public partial class Form2 : Form
    {
        ...
        public string connectString1 = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=Диспетчер.mdb;";
        private void Get_Table(string table_name, int num_dG)
        {
            Form1 f1 = new Form1();
            string CommandText = "SELECT * FROM ";
            CommandText += table_name;
            OleDbDataAdapter dataAdapter = new OleDbDataAdapter(CommandText, f1.connectString1);
            DataSet ds = new DataSet(); dataAdapter.Fill(ds, table_name);
        ...
        private void Form2_Load(object sender, EventArgs e)
        {
            ...
            Get_Table("Водитель", 3); Get_Table("Автобус", 4);
            Get_Table("Диспетчер", 5); textBox1.Text = "0";
        ...
        {
            int row; // Берем данные из ячеек таблицы "Маршрут"
        ...
        {
            int row; // так же берем данные из таблицы "Билет"
        ...
        {
            int row; // Информация об автобусе
        ...
        {
            int row; // Информация о диспетчере
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (textBox2.Text == "") CommandText = "SELECT * FROM [Маршрут]";
            else if (comboBox1.SelectedIndex == 0) // № перевозки 
            ...
            else if (comboBox1.SelectedIndex == 2) // Район 
                CommandText = "SELECT * FROM [Маршрут] WHERE Район LIKE '" + textBox2.Text + "%'";
            else if (comboBox1.SelectedIndex == 3) // Область
            CommandText = "SELECT * FROM [Маршрут] WHERE Область LIKE '" + textBox2.Text + "%'"; Form1 f = new Form1();
            OleDbDataAdapter dataAdapter = new OleDbDataAdapter(CommandText, f.connectString1);
            DataSet ds = new DataSet(); dataAdapter.Fill(ds, "[Маршрут]");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string CommandText = "SELECT * FROM [Билет]"; // формируем переменную CommandText
            if (textBox3.Text == "")
                CommandText = "SELECT * FROM [Билет]";
            else if (comboBox2.SelectedIndex == 0) // № перевозки
